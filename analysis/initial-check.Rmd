...
```{r pc1-pc2-flow-cell}
pc1pc2 %+% aes(color = flow_cell) + labs(title = "Flow cell")
```
...
```{r pc3-pc4-flow-cell}
pc3pc4 %+% aes(color = flow_cell) + labs(title = "Flow cell")
```
...
d <- dist(t(tpm_clean))
h <- hclust(d)
plot(h, labels = anno$treatment, main = "Treatment", cex = 0.75)
plot(h, labels = substr(anno$flow_cell, 3, 5), main = "Flow cell", cex = 0.75)
```

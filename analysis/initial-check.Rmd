...
gtext <- geom_text(aes(label = individual))
```

```{r pc1-pc2-treatment}
pc1pc2 %+% aes(color = treatment) + gtext + labs(title = "Treatment")
```

```{r pc1-pc2-susceptibility}
pc1pc2 %+% aes(color = susceptibility) + gtext + labs(title = "Susceptibility")
```

```{r pc1-pc2-lane}
pc1pc2 %+% aes(color = lane, shape = flow_cell) + geom_point() + labs(title = "Lane")
```

```{r pc1-pc2-flow-cell}
pc1pc2 %+% aes(color = flow_cell) + geom_point() + labs(title = "Flow cell")
```
...
```{r pc3-pc4-treatment}
pc3pc4 %+% aes(color = treatment) + gtext + labs(title = "Treatment")
```

```{r pc3-pc4-susceptibility}
pc3pc4 %+% aes(color = susceptibility) + gtext + labs(title = "Susceptibility")
```

## Hierarchical clustering

```{r dendrogram, fig.width=8}
d <- dist(t(tpm_clean))
h <- hclust(d)
plot(h, labels = anno$treatment, main = "Treatment", cex = 0.5)
plot(h, labels = substr(anno$flow_cell, 3, 5), main = "Flow cell", cex = 0.5)
```
